...
const char *hello_world_kernel = 
"__kernel                                   \n"
"void hello_world(__global int *A, int n)                         \n"
"{                                          \n"
"    //Get the index of the work-item       \n"
"    int x = get_local_id(0)+get_local_size(0)*get_group_id(0);              \n"
"    int y = get_local_id(1)+get_local_size(1)*get_group_id(1);              \n"
"    int k = get_local_id(2)+get_local_size(2)*get_group_id(2);              \n"
"    int index = k + y * get_global_size(2) + x * get_global_size(2) * get_global_size(1);              \n"
"    if (index < n) {          \n"
...
int main(void) {
  ...
  cl_command_queue command_queue = clCreateCommandQueue(context, device_list[0], 0, &err);
	...
	cl_program program = clCreateProgramWithSource(context, 1,(const char **)&hello_world_kernel, NULL, &err);
  ...
  err  = clSetKernelArg(kernel, 1, sizeof(int), &n);CHK_ERROR(err);
  ...
  global[0] = 16; global[1] = 8; global[2] = 2;
  local[0] = 8; local[1] = 4; local[2] = 1;

 	
	
  err = clEnqueueNDRangeKernel(command_queue, kernel, 3, NULL, global, local, 0, NULL, NULL); CHK_ERROR(err);
  ...
  clEnqueueReadBuffer(command_queue, d_A, CL_TRUE, 0,
