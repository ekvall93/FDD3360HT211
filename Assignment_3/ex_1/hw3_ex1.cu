...
__device__ int get_ix(int y, int x, int dy, int dx, int width) {
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    int sIx = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        int offset_t = index_y * width + index_x;
        int offset   = (index_y + 1) * width + (index_x + 1);
        
        sh_block[sIx]  = image[offset_t];

        /* Fill border */
        if (threadIdx.x == (BLOCK_SIZE - 1) || threadIdx.y == (BLOCK_SIZE - 1)) {
            for (int x=0; x<3; x++) {
                for (int y=0; y<3; y++) {
                    sh_block[get_ix(threadIdx.y, threadIdx.x, y, x, BLOCK_SIZE_SH)]  = image[get_ix(index_y, index_x, y, x, width)];
                }
            }
        }    
        __syncthreads();
        image_out[offset] = gpu_applyFilter(&sh_block[sIx],BLOCK_SIZE_SH, gaussian, 3);
    }
}
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        float gx = gpu_applyFilter(
            &sh_block[sIx], 
            BLOCK_SIZE_SH, sobel_x, 3);
        float gy = gpu_applyFilter(
            &sh_block[sIx], 
